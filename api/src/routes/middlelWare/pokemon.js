...
router.post("/", async (req, res) => {
    ...
    try {
        ...
        const cantidad = await Pokemon.findAll()
        console.log(cantidad.length)

        if (!Imagen) {
            var newPoke = await Pokemon.create({
                Nombre, Vida, Ataque, Defensa, Velocidad, Altura, Peso, Imagen: "https://st2.depositphotos.com/1906711/11944/v/600/depositphotos_119441904-stock-illustration-pokeball-hanging-in-the-air.jpg", IdFake: (40 + cantidad.length + 1)
            })
        } else {
            var newPoke = await Pokemon.create({
